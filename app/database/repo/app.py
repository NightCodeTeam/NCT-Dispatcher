...
class AppRepo(Repository):
    ...
    async def get_app_by_id(self, app_id: int, session: AsyncSession) -> App | None:
        return await self.get(_filter=f'{self.table_name}.id={app_id}', session=session)

    async def get_app_by_ip_name_code(self, app_id: int, name: str, code: str, session: AsyncSession) -> App | None:
    ...
    async def get_app_by_name_code(self, name: str, code: str, session: AsyncSession) -> App | None:
        return await self.get(
    ...
    async def add_app(self, name: str, url: str, dispatcher_code: str, session: AsyncSession, commit: bool = False) -> bool:
        try:
    ...
    async def delete_by_ip(self, app_id: int, session: AsyncSession, commit: bool = False) -> bool:
