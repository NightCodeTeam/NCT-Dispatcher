...
- Web-версия:
  ...
  - Добавлена возможность удаленно запускать приложения (beta)

## Компоненты

### Web версия

Для запуска ознакомительной версии подготовлен `docker-compose.yml` файл. Не забудьте настроить .env файлы в `frontend` и `backend` (изначально будут использоваться `.env.prod`).
...
```json
{
    ...
    "app_code": "your_generated_code_here",
}
```

## Внешние зависимости

Web версия Dispatcher использует:

 - [NCT-Blocker](https://github.com/NightCodeTeam/NCT-Blocker) - сервис для проверки ip адресов
 - NCT-Auth - сервис авторизации пользователей
 - [core](https://github.com/MaxBro12/core) - микросервисный пакет
 - redis - не обязательно, приложение работает и без него

### Использование с системой NCT-Auth

Если вам нужна защита и авторизация запросов обращайтесь к нашей команде за подключением к системе NCT-Auth.
