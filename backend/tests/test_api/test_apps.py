...
from src.database import App, DataBase


async def test_all_apps(test_client: AsyncClient, test_db: DataBase):
...
async def test_app_by_id(test_client: AsyncClient, test_db: DataBase):
...
async def test_new_success(test_client: AsyncClient, test_db: DataBase):
...
async def test_app_by_id_wrong_id(test_client: AsyncClient, test_db: DataBase):
    res = await test_client.get('/v1/apps/10')
    assert res.status_code == 404


async def test_app_logs(test_client: AsyncClient, test_db: DataBase):
    res = await test_client.get('/v1/apps/1/logs')
    assert res.status_code == 200
    assert type(res.json().get('logs')) is list


async def test_app_logs_wrong(test_client: AsyncClient, test_db: DataBase):
    res = await test_client.get('/v1/apps/100/logs')
    assert res.status_code == 404


async def test_del_app(test_client: AsyncClient, test_db: DataBase):
    app = App(
        id=16,
        name='DeleteApp',
        code='123',
